...
frappe.ui.form.on("Demande de financement", {
	refresh(frm) {
        frm.add_custom_button('Brouillon', () => {
            frm.set_value('statut', 'Brouillon')
                .then(() => {
                    frm.save()
            })
        }, __('Changer le statut'));
        frm.add_custom_button('En attente d\'envoie', () => {
            frm.set_value('statut', 'En attente d\'envoie')
                .then(() => {
                    frm.save()
            })
        }, __('Changer le statut'));
        frm.add_custom_button('En attente de validation', () => {
            frm.set_value('statut', 'En attente de validation')
                .then(() => {
                    frm.save()
            })
        }, __('Changer le statut'));
        frm.add_custom_button('Navette', () => {
            frm.set_value('statut', 'Navette')
                .then(() => {
                    frm.save()
            })
        }, __('Changer le statut'));
        ...
        frm.add_custom_button('En attente de paiement', () => {
            frm.set_value('statut', 'En attente de paiement')
                .then(() => {
                    frm.save()
            })
        }, __('Changer le statut'));
        frm.add_custom_button('Partiellement payé', () => {
            frm.set_value('statut', 'Partiellement payé')
                .then(() => {
                    frm.save()
            })
        }, __('Changer le statut'));
        frm.add_custom_button('Payé', () => {
            frm.set_value('statut', 'Payé')
                .then(() => {
                    frm.save()
            })
        }, __('Changer le statut'));
        ...
        frm.add_custom_button('En attente', () => {
            frm.set_value('statut', 'En attente')
                .then(() => {
                    frm.save()
            })
        }, __('Changer le statut'));
        frm.add_custom_button('Refusé', () => {
            frm.set_value('statut', 'Refusé')
                .then(() => {
                    frm.save()
            })
        }, __('Changer le statut'));
